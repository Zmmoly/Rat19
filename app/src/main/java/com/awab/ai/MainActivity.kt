...
class MainActivity : AppCompatActivity() {
    ...
    private fun extractCommandsFromText(text: String): List<String> {
        ...
        // أولاً: جرب تقسيم النص بالفواصل
        val separators = listOf("،", ",", "ثم", "\n", " و ", " ثم ")
        var segments = listOf(text)
        
        for (separator in separators) {
            if (text.contains(separator)) {
                segments = text.split(separator).map { it.trim() }.filter { it.isNotBlank() }
                break
            }
        }
        ...
        // إذا لم نجد أوامر في القطع، جرب النص الكامل
        if (commands.isEmpty()) {
            val hasCommand = commandKeywords.any { keyword ->
            ...
            if (hasCommand) {
                val cleanCommand = cleanTextForCommand(text)
                val response = commandHandler.handleCommand(cleanCommand)
                if (response != null && !response.contains("لم أفهم الأمر")) {
                    commands.add(cleanCommand)
                }
    ...
    private fun cleanTextForCommand(text: String): String {
        // إزالة الكلمات الإضافية الشائعة
        var cleaned = text
        
        val fillerWords = listOf(
            "لو سمحت", "من فضلك", "الرجاء", "ممكن", "أريد",
            "أرجو", "هل يمكن", "هل يمكنك", "تقدر", "ممكن تقدر",
